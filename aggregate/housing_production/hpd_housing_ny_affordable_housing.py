...
unit_income_levels = ['extremely_low_income_units',
...
def load_housing_ny():
    ...
    df = pd.read_csv('.library/hpd_hny_units_by_building.csv',
    usecols= ['project_id','project_name', 'project_start_date', 'project_completion_date', 'number',
...
def pivot_and_flatten_index(df, geography):
    ...
    df = df.pivot(index=geography, columns='reporting_construction_type' ,values=unit_income_levels)
...
def citywide_hny_units_con_type(df):
    ...
    results = df.groupby('reporting_construction_type')[unit_income_levels].sum().reset_index()
...
def borough_hny_units_con_type(df):
    ...
    results = df.groupby(['reporting_construction_type', 'borough'])[unit_income_levels].sum().reset_index()
...
if __name__ == '__main__':
    ...
    results_citywide.to_csv('internal_review/housing_production/affordable_housing_preservation_construction_citywide.csv', index = False)

    results_borough.to_csv('internal_review/housing_production/affordable_housing_preservation_construction_borough.csv', index = False)

    results_puma.to_csv('internal_review/housing_production/affordable_housing_preservation_construction_puma.csv', index = False)
