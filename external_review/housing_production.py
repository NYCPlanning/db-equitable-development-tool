...
for g in subfolders:
    ...
    csvs = [
        os.path.join(path + "/" + g, file) for file in files if file.endswith("csv")
    ]
    ...
    merged.rename(columns={"index": g}, inplace=True)

    merged.to_csv(
        "external_review/housing_production/housing_production_" + g + ".csv",
        index=False,
    )
