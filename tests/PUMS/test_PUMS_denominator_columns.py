...
def test_count_over_denom_is_fraction(indicator):
    ...
    assert (
        aggregated[f"{indicator}-count"] / aggregated[f"{indicator}-fraction-denom"]
        == aggregated[f"{indicator}-fraction"]
    ).all()
...
def test_default_denom_total_pop(indicator):
    ...
    aggregated = demographic_count_loader.aggregated
    assert (
        aggregated[f"{indicator}-fraction-denom"] == aggregated["total_pop-count"]
    ).all()


def test_LEP_denom_less_than_total_pop():
    aggregated = demographic_count_loader.aggregated
    assert (aggregated["lep-fraction-denom"] < aggregated["total_pop-count"]).all()
...
def tests_race_crosstab_denom_is_total_people_in_race_group(indicator):
    ...
    for race in races:
        assert (
            aggregated[f"{indicator}-{race}-fraction-denom"]
